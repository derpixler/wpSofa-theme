...
(async () => {
    await del([ path.resolve(__dirname, 'assets/dist/' + mode)]);
})();
...
module.exports = async ( env, argv ) => {
	...
	let Buildversion = hash( Date.now() ).substr( 0, 6 );
	...
	const config = {
		...
		output      : {
			path         : path.resolve( __dirname, 'assets/dist/' + mode + '/' + Buildversion + '/' ),
			...
			publicPath   : () => {
				...
				return host + arrayPath.reverse().join( "\/" ) + "/assets/dist/" + mode + '/' + Buildversion + '/';
			}
		},
		optimization: {
			minimize : true,
